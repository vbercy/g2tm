...
# ---------------------------------------------------------------
"""COCO dataset class for panoptic segmentation."""
...
class COCOPanoptic(LightningDataModule):
    """ADE20K dataset wrapper class for panoptic segmentation."""

    def __init__(
        self,
        path,
        stuff_classes: list[int],  # pylint: disable=W0613
        num_workers: int = 4,
        batch_size: int = 16,
    ...
    @staticmethod
    def target_parser(
        target, labels_by_id, is_crowd_by_id, **kwargs  # pylint: disable=W0613
    ):
        """Parse the masks and the labels from the groundtruth."""
    ...
    def setup(
        self, stage: Union[str, None] = None  # pylint: disable=W0613
    ) -> LightningDataModule:
        """Instanciate the dataset as two split (training and validation).

        Args:
            stage (str | None): Split of the dataset (deprecated).

        Returns:
            self: The object itself with the dataset instanciated.
        """
        dataset_kwargs = {
            "img_suffix": ".jpg",
            "target_suffix": ".png",
        ...
        self.train_dataset = Dataset(  # pylint: disable=W0201
        ...
        self.val_dataset = Dataset(  # pylint: disable=W0201
    ...
    def train_dataloader(self):
        ...
        Returns:
            DataLoader: the data loader of the training split.
        """
        return DataLoader(
            self.train_dataset,
            shuffle=True,
