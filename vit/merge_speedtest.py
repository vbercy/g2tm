...
def record_merge_inputs(records: list):
    ...
    Args:
        records (list): List filled with one (feat, distill_token) tuple
            per call to the merging function.
    """
    patch_module = importlib.import_module("g2tm.patch.graph_vit_patch")
    original = patch_module.bfs_merge

    def recorder(feat, threshold, distill_token=False):
        records.append((feat.detach().clone(), distill_token))
        return original(feat, threshold, distill_token)
...
def capture_features(
    ...
    Returns:
        List[tuple]: One (feat, distill_token) tuple per batch.
    """
    captured = []
    if n_images:
...
def time_implementation(
    ...
    for feat, distill_token in captured[: min(3, len(captured))]:
        merge_fn(feat.clone(), threshold, distill_token)
    if ptu.use_gpu:
        torch.cuda.synchronize()

    for feat, distill_token in tqdm(
        ...
        for _ in range(repeats):
            ...
            out = merge_fn(work, threshold, distill_token)
            if ptu.use_gpu:
                torch.cuda.synchronize()
...
def print_report(results: Dict[str, Dict[str, float]], n_calls: int, batch_size: int):
    ...
    Args:
        ...
        batch_size (int): Number of images per batch, as the number of tokens
            kept is only meaningful when the merged tokens are popped (b = 1).
    """
...
def main(
    ...
    g2tm.graph_vit_patch(model, selected_layer, threshold)
    model.eval()
    ...
    implementations = {
        ...
        "fast_sv_merge": lambda feat, thr, dist: fast_sv_merge(
            feat, thr, num_iters, dist
        ),
    }
    results = {}
