...
def record_merge_inputs(records: list):
    ...
    def recorder(  # pylint: disable=W0613
        feat, threshold, distill_token=False, unmerge_idx_needed=False
    ):
        records.append((feat.detach(), distill_token))
...
def compare_variants(
    ...
    for image in tqdm(loader, total=total, position=0, leave=False):
        ...
        feat, distill_token = records[0]
        protected = 1 + int(distill_token)
...
def main(
    ...
    g2tm.graph_vit_patch(model, selected_layer, threshold, prop_attn, iprop_attn)
    model.eval()
