...
def compute_flops_per_image_per_module(
    ...
    for image in tqdm(validation_loader, position=0, leave=False):
        ...
        encoded = model.vit(image, return_features=True)

        enc_flops = FlopCountAnalysis(model.vit, (image, True))
        (enc_flops.unsupported_ops_warnings(False).uncalled_modules_warnings(False))
        enc_gflops.append(enc_flops.total() / 1e9)

        dec_flops = FlopCountAnalysis(model.vit.head, encoded[:, 0])
...
def main(
    ...
    model.eval()
    model.to(device)
    print(parameter_count_table(model, 2))

    validation_loader = create_dataset(dataset_kwargs)

    if enc_dec:
        enc_gflops, head_gflops = compute_flops_per_image_per_module(
            model, validation_loader, device
        )
        head_params = parameter_count(model.vit.head)[""]
        enc_params = parameter_count(model)[""] - head_params
        print(f"Encoder:\n\tGFlops    : {enc_gflops:0.3f}")
        print(f"\tParameters: {enc_params / 1e6:0.3f}")
        print(f"Head:\n\tGFlops       : {head_gflops:0.3f}")
        print(f"\tParameters: {head_params / 1e6:0.3f}")
    else:
        gflops = compute_flops_per_image(model, validation_loader, device)
        params = parameter_count(model)[""]
        print(f"GFlops: {gflops:0.3f}")
        print(f"Parameters: {params / 1e6:0.3f}")
