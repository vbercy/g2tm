...
# ---------------------------------------------------------------
"""LighningModule wrapper for EoMT."""
...
class LightningModule(lightning.LightningModule):
    """LightningModule wrapper for a PyTorch model."""
    ...
    def __init__(
        ...
        patch_type: str = "pure",
        selected_layer: int = 2,
        threshold: float = 0.88,
        curric_thresh: bool = False,
        start_thresh: float = 0.95,
        curric_warmup: int = 6,
        curric_period: int = 3,
        ...
        self.curric_warmup = curric_warmup
        self.curric_period = curric_period
        self.prop_attn = prop_attn
        self.iprop_attn = iprop_attn
        self.method = method
        self.num_iters = num_iters
        if self.patch_type == "graph":
            tau = start_thresh if curric_thresh else threshold
            graph_eomt_patch(
                network, selected_layer, tau, prop_attn, iprop_attn, method, num_iters
            )
        elif self.patch_type != "pure":
    ...
    def configure_optimizers(self):
        """Configure the learning rate optimizer."""
        encoder_param_names = {
    ...
    def forward(self, imgs):  # pylint: disable=W0221
        """Forward method."""
        x = imgs / 255.0
    ...
    def validation_step(self, batch, batch_idx=0):  # pylint: disable=W0221
        """Validation step for a single batch."""
        return self.eval_step(batch, batch_idx, "val")

    def mask_annealing(self, start_iter, current_iter, final_iter):
        """Supervise the mask annealing strategy."""
        device = self.device
    ...
    def on_train_batch_end(
        self,
        outputs,
        batch,  # pylint: disable=W0613
        batch_idx=None,  # pylint: disable=W0613
        dataloader_idx=None,  # pylint: disable=W0613
    ):
        """Triggered at the end of the process of each batch during training step."""
    ...
    def on_train_epoch_start(self):
        ...
        if self.curric_thresh:
            if (
                self.current_epoch > self.curric_warmup - 1
                and (self.current_epoch - self.curric_warmup) % self.curric_period == 0
                and self.network.threshold > self.threshold
            ):
                self.network.threshold = round(self.network.threshold - 0.01, 2)
                self.network.info["threshold"] = round(
                    self.network.info["threshold"] - 0.01, 2
                )
                print(f"Reducing threshold to {self.network.threshold}.")

    def init_metrics_semantic(self, ignore_idx, num_blocks):
        """Instanciate the metrics for a semantic segmentation task."""
        self.metrics = nn.ModuleList(  # pylint: disable=W0201
            [
    ...
    def init_metrics_instance(self, num_blocks):
        """Instanciate the metrics for an instance segmentation task."""
        self.metrics = nn.ModuleList(  # pylint: disable=W0201
    ...
    def init_metrics_panoptic(self, thing_classes, stuff_classes, num_blocks):
        """Instanciate the metrics for a panoptic segmentation task."""
        self.metrics = nn.ModuleList(  # pylint: disable=W0201
            [
                PanopticQuality(
                    thing_classes,
    ...
    def update_metrics_semantic(
        ...
        """Update the semantic segmentation metrics."""
        for i, pred in enumerate(preds):
            self.metrics[block_idx].update(pred[None, ...], targets[i][None, ...])
    ...
    def update_metrics_instance(
        ...
        """Update the instance segmentation metrics."""
    ...
    def update_metrics_panoptic(
        ...
        """Update the panoptic segmentation metrics."""
        for i, pred in enumerate(preds):
    ...
    def _on_eval_epoch_end_semantic(self, log_prefix, log_per_class=False):
        for i, metric in enumerate(self.metrics):  # type: ignore
            ...
            if not block_postfix:
                self.log(f"metrics/{log_prefix}_alias", iou_all)
    ...
    def _on_eval_epoch_end_instance(self, log_prefix):
        for i, metric in enumerate(self.metrics):  # type: ignore
            ...
            if not block_postfix:
                self.log(f"metrics/{log_prefix}_alias", results["map"])
    ...
    def _on_eval_epoch_end_panoptic(self, log_prefix, log_per_class=False):
        for i, metric in enumerate(self.metrics):  # type: ignore
            ...
            block_postfix = self.block_postfix(i)
            if log_per_class:
                for class_idx in range(len(pq)):  # pylint: disable=C0200
                    self.log(
            ...
            self.log(f"metrics/{log_prefix}_pq_all{block_postfix}", pq.mean())
            ...
            if not block_postfix:
                self.log(f"metrics/{log_prefix}_alias", pq.mean())
    ...
    def plot_semantic(
        ...
        """Plot a layout of semantic segmentation maps (image, GT, prediction)."""
    ...
    def scale_img_size_semantic(self, size: tuple[int, int]):
        """Scale image size for the semantic segmentation task."""
        factor = max(
    ...
    def window_imgs_semantic(self, imgs):
        """Slice the image into several crops using a sliding window."""
        crops, origins = [], []
        ...
        for i, img in enumerate(imgs):
            ...
            resized_img = pil_img.resize(
                (new_w, new_h), Image.BILINEAR  # pylint: disable=E1101
            )
            resized_img = (
    ...
    def revert_window_logits_semantic(self, crop_logits, origins, img_sizes):
        """Combine the predictions of different crops into the original image size."""
    ...
    def to_per_pixel_logits_semantic(
        ...
        """Convert the predictions into the RGB format for the semantic segmentation
        task.
        """
        return torch.einsum(
            "bqhw, bqc -> bchw",
    ...
    def to_per_pixel_targets_semantic(
        ...
        """Convert the labels into the RGB format for the semantic segmentation task."""
        per_pixel_targets = []
    ...
    def scale_img_size_instance_panoptic(self, size: tuple[int, int]):
        """Scale image size for the panoptic segmentation task."""
        factor = min(
            self.img_size[0] / size[0],
            self.img_size[1] / size[1],
        )
    ...
    def resize_and_pad_imgs_instance_panoptic(self, imgs):
        """Resize and pad images for the panoptic segmentation task."""
        transformed_imgs = []
    ...
    def revert_resize_and_pad_logits_instance_panoptic(
        ...
        """Revert the resizing and the padding applied to the images for the panoptic
        segmentation task.
        """
        logits = []
        for i, tr_logits in enumerate(transformed_logits):
            scaled_size = self.scale_img_size_instance_panoptic(img_sizes[i])
            logits_i = tr_logits[:, : scaled_size[0], : scaled_size[1]]
    ...
    def to_per_pixel_preds_panoptic(
        ...
        """Convert the predictions into the RGB format for the panoptic segmentation
        task.
        """
    ...
    def to_per_pixel_targets_panoptic(targets: list[dict]):
        """Convert the labels into the RGB format for the panoptic segmentation task."""
        per_pixel_targets = []
        for target in targets:
    ...
    def on_load_checkpoint(self, checkpoint):
        """Triggered when loading a checkpoint."""
        if self.patch_type == "pure":
            if "merging" in checkpoint.keys():
                warnings.warn(
                    "Loaded checkpoint was previously trained with G2TM, "
                    "continuing without token merging as requested.",
                    RuntimeWarning,
                )
            return
        else:
            if not "merging" in checkpoint.keys():
                warnings.warn(
                    "Loaded checkpoint was not previously trained with G2TM, "
                    "starting token merging as requested.",
                    RuntimeWarning,
                )
                return

            merging_params = checkpoint["merging"]
            print(
                "WARNING: Loading model checkpoint originally trained with the following G2TM "
                f"parameters:\n{merging_params}\nPlease make sure it is coherent with "
                "what you have provided in the command line, as these parameters will "
                "overwrite the provided ones if the training was with curriculum and unfinished."
            )

            print("G2TM parameters of the loaded checkpoint:")
            print(f"\tProportional Attention activated: {merging_params['prop_attn']}")
            print(
                "\tInverse Proportional Attention activated: "
                f"{merging_params['iprop_attn']}"
            )
            print(
                f"\tG2TM implementation: {merging_params['method']}"
                + (
                    f"({merging_params['num_iters']} iterations)"
                    if merging_params["method"] == "fastsv"
                    else ""
                )
            )

            if (
                merging_params["curric_thresh"]
                and merging_params["end_thresh"] != merging_params["current_thresh"]
            ):
                print(
                    "Overwriting the provided G2TM threshold according to the "
                    "loaded checkpoint."
                )

                self.start_thresh = merging_params["start_thresh"]
                self.curric_warmup = merging_params["curric_warmup"]
                self.curric_period = merging_params["curric_period"]
                self.threshold = merging_params["end_thresh"]
                self.network.threshold = merging_params["current_thresh"]
                self.network.info["threshold"] = self.network.threshold

    def on_save_checkpoint(self, checkpoint):
        """Triggered when saving a checkpoint."""
        checkpoint["state_dict"] = {
            k.replace("._orig_mod", ""): v for k, v in checkpoint["state_dict"].items()
        }
        if self.patch_type != "pure":
            checkpoint["merging"] = {
                "curric_thresh": self.curric_thresh,
                "start_thresh": self.start_thresh if self.curric_thresh else None,
                "end_thresh": self.threshold,
                "current_thresh": self.network.threshold,
                "curric_warmup": self.curric_warmup,
                "curric_period": self.curric_period,
                "prop_attn": self.prop_attn,
                "iprop_attn": self.iprop_attn,
                "method": self.method,
        ...
        with torch.no_grad():
            ...
            for name, p in self.named_parameters():
                ...
                if not name.startswith(encoder_prefix):
                    ...
                    zeroed += p.numel()
            msg = (
                f"Zeroed {zeroed:,} / {total:,} parameters (everything not under "
                f"'{encoder_prefix}'"
            )
            if skip_class_head:
    ...
    def _load_ckpt(self, ckpt_path, load_ckpt_class_head):
        ...
        logging.info("Loaded %d keys", len(ckpt))
        return ckpt
