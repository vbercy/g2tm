...
On this branch, Graph-Guided Token Merging (G2TM) is applied to
...
**NOTE:** To run G2TM on other available models (SETR and EoMT for semantic segmentation, ViT for image classification), please change branch.
...
``` bash
git clone https://github.com/vbercy/g2tm --branch segm
cd g2tm
```
...
``` bash
# Create environment
conda create -n g2tm_segm python==3.11.*
conda activate g2tm_segm
...
from g2tm.patch import graph_segmenter_patch
device = 'cuda' if torch.cuda.is_available() else 'cpu'
encoder = VisionTransformer((128, 128), 16, 4, 192, 768, 3, 19)
decoder = create_decoder(encoder, {'name': 'mask_transformer', 'n_cls': 19, 'n_layers': 1, 'dropout': 0.0, 'drop_path_rate': 0.0})
model = Segmenter(encoder, decoder, n_cls=19).eval().to(device)
graph_segmenter_patch(model, selected_layer=2, threshold=0.88)
...
python ./segm/scripts/prepare_cityscapes.py <cityscapes_dir> --username <username> --password <password>
...
To train a Segmenter model (size tiny, small, base or large) with G2TM on a specific dataset (whose path is provided by `DATASET`), use the command provided below. For example, we chose to apply G2TM at the 2nd layer with a threshold of 0.88 and without any modified attention formulation. We recommand using the Fast SV implementation of G2TM here, as it is the fastest implementation when running a batched training.
...
To export a specific model into the ONNX format and evaluate it using ONNX Runtime, use the command provided below. In the example below, we convert a a Segmenter model with G2TM applied at the 2nd layer with a threshold of 0.88 into an ONNX file. G2TM hyperparameters will be frozen inside the file.

**NOTE:** The "ONNX-friendly" implementation of G2TM, using FastSV algorithm, will be automatically selected.

```bash
python ./segm/export_onnx.py <model_path> \
       --onnx-name <file_name> \
       --patch-type graph \
       --selected-layer 2 \
       --threshold 0.88 \
...
```bash
python ./segm/fastsv_iters.py <model_path> <dataset_name> \
       --selected-layer 2 \
       --threshold 0.88 \
       --max-iters 10 \
       [--n-images N]
