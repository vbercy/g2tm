...
Below, we provide the training commands for training SETR + G2TM.
...
Different backbone sizes are available for SETR using the `backbone` option:
* SETR with ViT-T (SETR-T): `vit_tiny_patch16_384`
* SETR with ViT-S (SETR-S): `vit_small_patch16_384`
* SETR with ViT-B (SETR-B): `vit_base_patch16_384`
* SETR with ViT-L (SETR-L): `vit_large_patch16_384`

For more configuration options, see [setr/config.yml](setr/config.yml) and [setr/train.py](setr/train.py).
...
The following commands enable single-GPU training with different ViT backbone sizes and decoders.

SETR-Naive + G2TM at layer 2 with threshold 0.88, and ViT-T backbone:

```bash
python ./setr/train.py --log-dir <tiny_model_dir> \
                       --dataset ade20k \
...
SETR-PUP + G2TM at layer 2 with threshold 0.88, and ViT-S backbone:

```bash
python ./setr/train.py --log-dir <small_model_dir> \
                       --dataset ade20k \
...
SETR-MLA + G2TM at layer 2 with threshold 0.88, and ViT-B backbone:

```bash
python ./setr/train.py --log-dir <base_model_dir> \
                       --dataset ade20k \
...
SETR-Naive + G2TM at layer 2 with threshold 0.88, and ViT-L backbone:

```bash
python ./setr/train.py --log-dir <large_model_dir> \
                       --dataset ade20k \
...
SETR-Naive + G2TM at layer 2 with threshold 0.94, ViT-L backbone, with 2 GPUs:
...
SETR-Naive + G2TM at layer 2 with threshold 0.94, and ViT-S backbone:

```bash
python ./setr/train.py --log-dir <citylarge_model_dir> \
...
SETR-Naive + G2TM at layer 2 with threshold 0.88, with Proportional Attention and ViT-S backbone:

```bash
python ./setr/train.py --log-dir <pa_model_dir> \
                       --dataset ade20k \
                       --backbone vit_small_patch16_384 \
                       --decoder naive \
                       --patch-type graph \
...
SETR-Naive + G2TM at layer 2 with threshold 0.88, with Inverse Proportional Attention and ViT-S backbone:

```bash
python ./setr/train.py --log-dir <ipa_model_dir> \
                       --dataset ade20k \
                       --backbone vit_small_patch16_384 \
                       --decoder naive \
                       --patch-type graph \
...
SETR-Naive + G2TM at layer 2 with threshold 0.88, with curriculum and ViT-S backbone:

```bash
python ./setr/train.py --log-dir <curric_model_dir> \
                       --dataset ade20k \
                       --backbone vit_small_patch16_384 \
                       --decoder naive \
                       --patch-type graph \
