...
# ---------------------------------------------------------------
"""Vision Transformer class, as EoMT encoder, created using either `timm` or
`transformers` and returned as a timm model.
"""
...
class ViT(nn.Module):  # pylint: disable=W0223
    """Vision Transformer returned as a `timm` model."""
    ...
    def transformers_to_timm(self, backbone, img_size: tuple[int, int]):
        """Convert a `transformers` model to a `timm` model."""
