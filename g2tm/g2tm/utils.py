...
def g2tm_merge(
    ...
    Args:
        ...
        threshold (float): Threshold parameter for G2TM.
        is_encoder (bool): Whether the G2TM have been inserted in a ViT encoder.
        distill_token (bool): Whether the sequence contains a distillation token.
    ...
    # Merge tokens (static shapes, batched)
    x_feat, size, mask = merge_from_labels(x_feat, labels, b, n, d, device)
    ...
    unmerge_idx = get_unmerge_idx(labels, mask, sparse=b == 1)

    if b == 1:
        # Reduced tokens are popped
        x_feat, size = sparsify(x_feat, size, mask)
        mask = None
    else:
        # Reduced tokens are kept in place and masked
        mask = torch.cat(  # pylint: disable=E1101
            (
                torch.ones(  # pylint: disable=E1101
                    b, protected, dtype=torch.bool, device=device
                ),
                mask,
            ),
            dim=1,
    ...
    size = torch.cat(  # pylint: disable=E1101
        (
            torch.ones(  # pylint: disable=E1101
                b, protected, dtype=torch.int64, device=device
            ),
            size,
        ),
        dim=1,
    ...
    return feat, size, mask, unmerge_idx
