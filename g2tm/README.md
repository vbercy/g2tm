...
In this section, we will explain how to set the environment up for this repository (SETR + G2TM) step by step. 
...
``` bash
# Create environment
conda create -n g2tm_setr python==3.11.*
conda activate g2tm_setr
...
**3. Installing the SETR requirements:**

``` bash
# Install the OpenMMLab stack. No prebuilt mmcv wheel exists above Torch 2.4, so mmcv is
# compiled from source (~5 min), its CUDA operators using the system CUDA toolkit.
pip install --no-build-isolation mmcv==2.1.0
pip install mmsegmentation==1.2.2
# set up the SETR package
pip install -v -e .
```
...
The environment is ready when the following command prints the shape of a segmentation map:
...
```bash
python ./setr/scripts/prepare_ade20k.py <ade20k_dir>
python ./setr/scripts/prepare_cityscapes.py <cityscapes_dir> --username <username> --password <password>
python ./setr/scripts/prepare_pcontext.py <pcontext_dir>
```
...
```bash
python ./setr/test.py <ckpt_file> \
       --patch-type graph \
       --selected-layer 2 \
       --threshold 0.88
```

Note that you can still use the evaluation script (mIoU, mAcc, pAcc) provided by SETR using this command:

```bash
# single-scale baseline + G2TM evaluation:
python ./setr/eval/miou.py <ckpt_file> <dataset_name> \
       --singlescale \
       --patch-type graph \
       --selected-layer 2 \
       --threshold 0.88
...
```bash
# Im/sec
python ./setr/speedtest.py <ckpt_file> <dataset_name> \
       --batch-size 1 \
       --patch-type graph \
       --selected-layer 2 \
       --threshold 0.88
...
```bash
# GFLOPs
python ./setr/flops.py <ckpt_file> <dataset_name> \
       --batch-size 1 \
       --patch-type graph \
       --selected-layer 2 \
       --threshold 0.88
...
```bash
python ./setr/profile_model.py <ckpt_file> <dataset_name> \
       --patch-type graph \
       --selected-layer 2 \
       --threshold 0.88
```
...
We explain here the specific options for G2TM:
- `--onnx-name <file_name>`: Name of the ONNX file (without the .onnx extension).
...
See [RESULTS](./RESULTS.md) for some comparative results for SETR + G2TM and the corresponding model checkpoints.
...
All files covered by SETR's, Segmenter's or ToMe's licences include a header indicating the licence and whether the file has been modified. You can find such files from SETR's and Segmenter's repository in the [`setr`](./setr/) directory and from ToMe's repository in the [`patch`](./g2tm/g2tm/patch/) and [`vis`](./g2tm/g2tm/vis/) folders.
