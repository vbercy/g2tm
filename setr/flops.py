...
from setr.model.utils import padding
...
def compute_flops_per_image_per_module(
    ...
    for image in tqdm(validation_loader, position=0, leave=False):
        ...
        encoded = model.encoder(image, model.token_reduction)
        ...
        dec_flops = FlopCountAnalysis(model.decoder, (*encoded, (h, w)))
...
def main(
    ...
    if enc_dec:
        ...
        enc_params = parameter_count(model.encoder)[""]
        dec_params = parameter_count(model.decoder)[""]
        print(f"Encoder:\n\tGFlops    : {enc_gflops:0.3f}")
        print(f"\tParameters: {enc_params / 1e6:0.3f}")
        print(f"Decoder:\n\tGFlops    : {dec_gflops:0.3f}")
        print(f"\tParameters: {dec_params / 1e6:0.3f}")
    else:
