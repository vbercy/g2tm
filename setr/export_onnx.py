...
from setr.utils.logger import MetricLogger
from setr.model.factory import load_model
from setr.model.utils import resize, sliding_window, merge_windows
from setr.data.utils import IGNORE_LABEL
from setr.data.factory import create_dataset
from setr.metrics import gather_data, compute_metrics
    ...
    with torch.no_grad():
        if onnx_name is None:
            onnx_name = (
                ...
                + f"_setr_{variant['net_kwargs']['decoder']['name']}"
                + f"_{size_letters[variant['net_kwargs']['backbone'].split('_')[1]]}"
